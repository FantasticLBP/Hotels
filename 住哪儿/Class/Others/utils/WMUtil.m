...
+ (CGSize)getHeighWithLabel:(NSString *)labelTitle labelTextFunt:(UILabel *)label{
    ...
    CGSize titleSize = [labelTitle boundingRectWithSize:CGSizeMake(BoundWidth, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:17]} context:nil].size;
    
    return titleSize;
    
    //content.frame = CGRectMake(kInterval_5, CGRectGetMaxY(titleLabel.frame) + kInterval_5, BoundWidth - 20, titleSize.height);
